...
void RightUnit::update(float f)
{
	...
	Rect enemyFacRect = Rect(0, enemyFactoryPos.y, 200, enemyFactory->return_Factory_Sp()->getContentSize().height);

	//내가 제일 앞에 있는 유닛일 경우
	if (unitNumber == GameManager::GetInstance()->frontUnitRight)
	{
		if (fullHP == nullptr)
			createHpBar();

		for (Unit* enemy : unitsL)
		{
			//가장 앞에 있는 적군과 충돌할 경우
			if (enemy->unitNumber == GameManager::GetInstance()->frontUnitLeft)
			{
